...
def main():
    p = subprocess.Popen("run-win-dnncli-module", shell=True)
    ...
    # 新規作成
    f = open('userUtteranceInfo.csv', 'w')
    f.write('u_s,word\n')
    f.close()
        ...
        try:
            ...
            if len(message) > 1:
                ...
                if ts != None:
                    ...
                    # 書き出し（追記）
                    f = open('userUtteranceInfo.csv', 'a')
                    f.write('{},{}\n'.format(str(us_time), ts.word.encode("utf-8")))
                    f.close()
                print("---------------------------------------")
...
def makeTSdic(xmlstr):
    ...
    # ださい
    retstr = retstr.replace("<", "").replace('"', '').replace(">", "").replace("/", "")
